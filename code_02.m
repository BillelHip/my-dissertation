...
for t = 2:numTime
	if(t==200)
		disp(t);
		Ex(t) = mean([Ex(t-1) 0.1]); % 0.5
		disp(Ex(t-1));disp('---');disp(mean([Ex(t-1) 0.1]));
		Er(t) = mean([Er(t-1) 0.1]); % 0.8
		Sc(t) = mean([Sc(t-1) 0.8]); % 0.2
		Py(t) = mean([Py(t-1) 0.1]); % 0.9
		Tp(t) = mean([Tp(t-1) 0.7]); % 0.1
		Abnorm(t) = mean([Abnorm(t-1) 0.4]); % 0.8
		Jlbasic(t) = mean([Jlbasic(t-1) 0.9]); % 0.4
		for n = t+1:numTime
			Ex(n) = Ex(n-1);
			Er(n) = Er(n-1);
			Sc(n) = Sc(n-1);
			...
			Tp(n) = Tp(n-1);
			Abnorm(n) = Abnorm(n-1);
			Jlbasic(n) = Jlbasic(n-1);
		end
	elseif (t==600)
		disp(t);
		Ex(t) = mean([Ex(t-1) 0.9]); % 0.5
		Er(t) = mean([Er(t-1) 0.9]); % 0.8
		Sc(t) = mean([Sc(t-1) 0.2]); % 0.2
		Py(t) = mean([Py(t-1) 0.9]); % 0.9
		Tp(t) = mean([Tp(t-1) 0.1]); % 0.1
		Abnorm(t) = mean([Abnorm(t-1) 0.9]); % 0.8
		Jlbasic(t) = mean([Jlbasic(t-1) 0.9]); % 0.4
		for n = t+1:numTime
			Ex(n) = Ex(n-1);
			Er(n) = Er(n-1);
	...
	elseif (t==800)
		...
		Er(t) = mean([Er(t-1) 0.1]); % 0.8
		Sc(t) = mean([Sc(t-1) 0.8]); % 0.2
		Py(t) = mean([Py(t-1) 0.4]); % 0.9
		Tp(t) = mean([Tp(t-1) 0.7]); % 0.1
		Abnorm(t) = mean([Abnorm(t-1) 0.1]); % 0.8
		Jlbasic(t) = mean([Jlbasic(t-1) 0.1]); % 0.4
		for n = t+1:numTime
			Ex(n) = Ex(n-1);
			Er(n) = Er(n-1);
			Sc(n) = Sc(n-1);
